...
if [ "x${1}" = "xupstart" ];then
    for j in \
        filesystem virtual-filesystems \
        net-device-up local-filesystems \
        remote-filesystems;do
    ...
    #for j in \
    #   apport resolvconf acpid udevtrigger \
    #   udev networking mount-all hostname tty{1,2,3,4,5,6,7,8,9};do
    for j in \
        apport acpid udevtrigger udev mount-all \
        hostname tty{1,2,3,4,5,6,7,8,9};do
...
if [ -f /sbin/cops_container_cleanup.sh ];then
    /sbin/cops_container_cleanup.sh || /bin/true
fi
